...
use Sam\EntityMaker\Auction\Validate\Internal\Date\Internal\Validate\Live\LiveDateValidationInput as Input;
use Sam\EntityMaker\Auction\Validate\Internal\Date\Internal\Validate\Live\LiveDateValidationResult as Result;

/**
 * Class LiveDateValidator
 * @package Sam\EntityMaker\Auction\Validate\Internal\Date\Internal\Validate\Live
 */
class LiveDateValidator extends CustomizableClass
{
    /**
    ...
    public function validate(Input $input): Result
    {
        ...
        if (!$input->auctionId) {
            ...
            $result = $validationHelper->checkRequired($input->liveEndDate, Result::ERR_LIVE_END_DATE_REQUIRED, $result);
        }
        ...
        $result = $validationHelper->checkNotEmpty($input->liveEndDate, Result::ERR_LIVE_END_DATE_REQUIRED, $result);
        ...
        $result = $validationHelper->checkDate($input->liveEndDate, Result::ERR_LIVE_END_DATE_INVALID, $input->mode, $result);
        ...
        if (
            ...
            && !$result->hasErrorByCode(Result::ERR_LIVE_END_DATE_INVALID)
        ) {
            $result = $validationHelper->checkDateLaterThan(
                $input->startClosingDate,
                $input->liveEndDate,
                Result::ERR_LIVE_END_DATE_EARLIER_START_CLOSING_DATE,
                $input->mode,
                $result
            );
        }
