...
use Sam\EntityMaker\User\Lock\Email\Internal\Detect\UniqueEmailLockRequirementCheckerCreateTrait;
use Sam\User\Lock\UserModificationLockerCreateTrait;

/**
 * Class UserUniqueEmailLocker
 * @package Sam\EntityMaker\User\Lock\Email
 */
class UserUniqueEmailLocker extends CustomizableClass
{
    use UniqueEmailLockRequirementCheckerCreateTrait;
    ...
    public function lock(
        ...
        $email = Cast::toString($inputDto->email);
        $userId = Cast::toInt($inputDto->id);
        $requirementCheckingResult = $this->createUniqueEmailLockRequirementChecker()->check($userId, $email);
        if ($requirementCheckingResult->mustLock()) {
            log_debug($requirementCheckingResult->message());
            $userModificationLocker = $this->createUserModificationLocker();
            $isLocked = $userModificationLocker->lock();
            if (!$isLocked) {
                return LockingResult::new()->canNotLock(self::class, $configDto);
            }
            $configDto->dbLocks[self::class] = $userModificationLocker->getLockName();
            return LockingResult::new()->locked(self::class, $configDto);
        }
