...
class QueryBuilder extends CustomizableClass
{
    ...
    protected string $sortOrderDefaultIndex = 'created_on';

    /** @var string[] */
    protected array $resultFieldsMapping = [
        'username' => 'u.username AS username',
        'user_id' => 'u.id AS user_id',
        'created_on' => 'u.created_on AS created_on',
        'referrer' => 'ui.referrer AS referrer',
    ...
    protected function buildQueryParts(): void
    {
        $this->queryParts = [
            ...
            'select_count' => 'SELECT COUNT(1) AS referrers_total FROM ( SELECT u.id %s ) AS cnt',
    ...
    {
        $n = "\n";
        $join = " LEFT JOIN `user` AS u ON ui.user_id = u.id";
    ...
    protected function getWhereClause(): string
    {
        ...
        if ($this->getReferrerHost() === '') {
            $conditions = "ui.referrer_host = ''";
        } else {
            $conditions = 'ui.referrer_host = ' . $this->escape($this->referrerHost);
        }

        if ($accountId) {
            $conditions .= ' AND (u.account_id = ' . $this->escape($accountId) . ')';
        }
        $conditions .= ' AND (u.user_status_id = ' . Constants\User::US_ACTIVE . ')';
        ...
        if ($this->isCollectedChecked() || $this->isPurchasedChecked()) {
            ...
            $userPeriodCond = ' AND (u.created_on BETWEEN ' . $this->escape($dateFrom) .
                ' AND ' . $this->escape($dateTo) . ')' . $n;
