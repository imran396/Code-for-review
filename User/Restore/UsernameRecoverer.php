<?php
/**
 * SAM-4181: Password and Username recoverer https://bidpath.atlassian.net/browse/SAM-4181
 *
 * @author        Imran Rahman
 * @version       SVN: $Id: $
 * @since         April 16, 2018
...
/**
 * Class UsernameRecoverer
 * @package Sam\User\Restore
 */
class UsernameRecoverer extends CustomizableClass
{
    ...
    public const ERR_COMBINATION_NOT_FOUND = 3;

    public const OK_SENT = 11;

    protected ?string $email = null;
    protected bool $captchaEnabled = true;
    ...
    public function initInstance(): static
    {
        ...
        $langUsernameEmailSentCb = static function () use ($tr) {
            return $tr->translate('FORGOT_USERNAME_EMAIL_SENT', 'login');
        };
        $errorMessages = [
        ...
        $successMessages = [
            self::OK_SENT => $langUsernameEmailSentCb,
        ];
     ...
     * @return static
     */
    public function disableCaptcha(bool $captchaEnabled = false): static
    {
     ...
     * user validation
     */
    public function validateUserAbsent(): void
    {
        if (!$this->getUser()) {
    ...
    {
        $errorMessage = (string)$this->getResultStatusCollector()->findFirstErrorMessageAmongCodes($this->emailErrors);
        return $errorMessage;
    }
     ...
     * @return string
     */
    public function alternateCaptchaErrorMessage(): string
    {
    ...
    public function notifyUser(?int $editorUserId): void
    {
        ...
        $emailManager = Email_Template::new()->construct(
            $accountId,
            Constants\EmailKey::FORGOT_USER,
            $editorUserId,
            [$this->getUser()]
        );
     ...
     * @return User|null
     */
    protected function loadUserByEmail(): ?User
    {
