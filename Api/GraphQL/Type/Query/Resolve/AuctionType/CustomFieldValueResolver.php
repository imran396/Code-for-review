...
class CustomFieldValueResolver extends CustomizableClass implements FieldResolverInterface
{
    ...
    protected AuctionCustField $customField;
    ...
    public function construct(AuctionCustField $customField): static
    {
    ...
    protected function renderValue(array $objectValue): int|float|bool|string|null
    {
        ...
        $value = match ($this->customField->Type) {
            Constants\CustomField::TYPE_DECIMAL => ((string)$value !== '')
                ...
                : null,
            Constants\CustomField::TYPE_FILE => $this->renderFile((string)$value, (int)$objectValue['id']),
    ...
    protected function renderFile(string $value, int $auctionId): string
    {
        $rendered = [];
        $files = explode('|', $value);
