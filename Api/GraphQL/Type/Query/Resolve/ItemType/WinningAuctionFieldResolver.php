...
/**
 * Class WinningAuctionFieldResolver
 * @package Sam\Api\GraphQL\Type\Query\Resolve\ItemType
 */
class WinningAuctionFieldResolver extends CustomizableClass implements FieldResolverInterface
{
    ...
    {
        return ['winning_auction_id'];
    }

    public function resolve(array $objectValue, array $args, AppContext $appContext, ResolveInfo $info): mixed
    {
        if ($objectValue['winning_auction_id'] === null) {
            return null;
        }

        $auctionData = ['id' => $objectValue['winning_auction_id']];
        if ($this->createFieldResolverHelper()->hasEnoughDataToResolve($info, $auctionData)) {
            return $auctionData;
        }
        return $appContext->dataLoader->loadAuction((int)$objectValue['winning_auction_id']);
    }
}
