...
class DataManager extends CustomizableClass implements IDataManager
{
    ...
    /**
     * Lock auction entry inside transaction
     * @param int $auctionId
     */
    public function lockInTransaction(int $auctionId): void
    {
        $query = <<<SQL
SELECT modified_on FROM auction
WHERE id = "{$this->escape($auctionId)}"
FOR UPDATE  
SQL;
        $this->nonQuery($query);
    }
