...
namespace Sam\Reminder\Concrete\Registration\Internal\Expiry;
...
class ExpirationDateRepository extends CustomizableClass
{
     ...
     */
    public function readLastRunDate(): ?DateTime
    {
        $dateIso = $this->createExpiryFileManager()
            ->readLastRunTimeOrInitByCurrentDate($this->makeFileRootPath());
     ...
     */
    public function writeLastRunDate(?DateTime $lastRunUtc, ?DateTime $reminderLastRunUtc): void
    {
        ...
        $message = $success
            ? sprintf(
                'Registration reminder last run timestamp changed from "%s" to "%s"',
                $reminderLastRunUtc ? $reminderLastRunUtc->format(Constants\Date::ISO) : '',
                $lastRunUtc ? $lastRunUtc->format(Constants\Date::ISO) : ''
            )
            : sprintf(
                'Registration reminder last run timestamp kept the same "%s"',
                $reminderLastRunUtc ? $reminderLastRunUtc->format(Constants\Date::ISO) : ''
            );
        log_debug($message);
    }
    ...
    public function makeFileRootPath(): string
    {
        return $this->createExpiryFileManager()
            ->makeFileRootPath($this->cfg()->get('core->reminder->registration->lastRunFile'));
    }
}
