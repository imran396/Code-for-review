...
/**
 * Command calls a checker that detects users with non-unique customer numbers
 *
 * Class UserCustomerNoUniqueCheckerCommand
 * @package Sam\DataIntegrityChecker\Cli
 */
class UserCustomerNoUniqueCheckerCommand extends CommandBase
{
    use UserDataIntegrityCheckerAwareTrait;

    public const NAME = 'UserCustomerNoUnique';

    private const MESSAGE_DUPLICATES = 'CustomerNo \'%s\' is duplicated %s times for user ids (%s)';
    private const MESSAGE_NO_DUPLICATES = 'There are no duplicated customer numbers';
    ...
    {
        parent::configure();
        $this->setDescription('Shows users with non-unique customer numbers');
    }
    ...
    private function yieldCustomFieldDuplicateData(): Generator
    {
        return $this->getUserDataIntegrityChecker()
            ->prepareCustomerNoDuplicateSearch()
            ->yieldRows();
    }
}
