...
/**
 * Command calls a checker that detects users with non-unique names
 *
 * Class UserUsernameUniqueCheckerCommand
 * @package Sam\DataIntegrityChecker\Cli
 */
class UserUsernameUniqueCheckerCommand extends CommandBase
{
    use UserDataIntegrityCheckerAwareTrait;

    public const NAME = 'UserUsernameUnique';

    private const MESSAGE_DUPLICATES = 'Username \'%s\' is duplicated %s times for user ids (%s)';
    private const MESSAGE_NO_DUPLICATES = 'There are no duplicated usernames';
    ...
    {
        parent::configure();
        $this->setDescription('Shows users with non-unique names');
    }
    ...
    private function yieldCustomFieldDuplicateData(): Generator
    {
        return $this->getUserDataIntegrityChecker()
            ->prepareUsernameDuplicateSearch()
            ->yieldRows();
    }
}
