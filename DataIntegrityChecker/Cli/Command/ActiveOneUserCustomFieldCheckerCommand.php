...
/**
 * Command calls a checker that detects custom fields that active more than once for one user
 *
 * Class ActiveOneUserCustomFieldCheckerCommand
 * @package Sam\DataIntegrityChecker\Cli
 */
class ActiveOneUserCustomFieldCheckerCommand extends CommandBase
{
    use UserDataIntegrityCheckerAwareTrait;

    public const NAME = 'ActiveOneUserCustomField';

    private const MESSAGE_CUSTOM_FIELD = 'Custom field \'%s\' is active for user %s %s times';
    ...
    {
        $this->setDescription('Shows custom fields active more than once for one user');
    }
