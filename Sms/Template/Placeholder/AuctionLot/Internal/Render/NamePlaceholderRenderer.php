...
class NamePlaceholderRenderer extends CustomizableClass implements PlaceholderRendererInterface
{
    ...
    public function render(SmsTemplatePlaceholder $placeholder, AuctionLotItem $auctionLot): string
    {
        ...
        $lotItem = $this->getDataProvider()->loadLotItem($auctionLot->LotItemId, true);
        $auction = $this->getDataProvider()->loadAuction($auctionLot->AuctionId, true);
        return $this->getLotRenderer()->makeName($lotItem->Name ?? '', $auction->TestAuction ?? false);
    }
}
