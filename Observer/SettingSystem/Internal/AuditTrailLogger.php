...
namespace Sam\Observer\SettingSystem\Internal;
 ...
 * @package Sam\Observer\SettingSystem\Internal
 */
    ...
    implements EntityCreationObserverHandlerInterface, EntityUpdateObserverHandlerInterface
{
    ...
    {
        /** @var SettingSystem $settingSystem */
        $settingSystem = $subject->getEntity();
        ...
        if ($message !== '') {
            ...
            $message = 'System Parameters (' . $settingSystem->AccountId . ') modified: ' . $message;
            $editorUserId = $this->getUserLoader()->loadSystemUserId();
            $this->getAuditTrailLogger()
                ->setAccountId($settingSystem->AccountId)
                ->setEditorUserId($editorUserId)
                ->setEvent($message)
                ->setSection($section)
                ->setUserId($settingSystem->ModifiedBy)
                ->log();
        }
    }
}
