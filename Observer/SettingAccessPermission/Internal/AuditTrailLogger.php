...
namespace Sam\Observer\SettingAccessPermission\Internal;
 ...
 * @package Sam\Observer\SettingAccessPermission\Internal
 */
    ...
    implements EntityCreationObserverHandlerInterface, EntityUpdateObserverHandlerInterface
{
    ...
    {
        /** @var SettingAccessPermission $settingAccessPermission */
        $settingAccessPermission = $subject->getEntity();
        ...
        if ($message !== '') {
            ...
            $message = 'System Parameters (' . $settingAccessPermission->AccountId . ') modified: ' . $message;
            $editorUserId = $this->getUserLoader()->loadSystemUserId();
            $this->getAuditTrailLogger()
                ->setAccountId($settingAccessPermission->AccountId)
                ->setEditorUserId($editorUserId)
                ->setEvent($message)
                ->setSection($section)
                ->setUserId($settingAccessPermission->ModifiedBy)
                ->log();
        }
    }
}
