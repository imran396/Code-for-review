...
namespace Sam\Observer\SettingSmtp\Internal;
 ...
 * @package Sam\Observer\SettingSmtp\Internal
 */
    ...
    implements EntityCreationObserverHandlerInterface, EntityUpdateObserverHandlerInterface
{
    ...
    {
        /** @var SettingSmtp $settingSmtp */
        $settingSmtp = $subject->getEntity();
        ...
        if ($message !== '') {
            ...
            $message = 'System Parameters (' . $settingSmtp->AccountId . ') modified: ' . $message;
            $editorUserId = $this->getUserLoader()->loadSystemUserId();
            $this->getAuditTrailLogger()
                ->setAccountId($settingSmtp->AccountId)
                ->setEditorUserId($editorUserId)
                ->setEvent($message)
                ->setSection($section)
                ->setUserId($settingSmtp->ModifiedBy)
                ->log();
        }
    }
}
