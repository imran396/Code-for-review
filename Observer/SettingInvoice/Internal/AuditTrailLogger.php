...
namespace Sam\Observer\SettingInvoice\Internal;
    ...
    implements EntityCreationObserverHandlerInterface, EntityUpdateObserverHandlerInterface
{
    ...
    {
        /** @var \SettingInvoice $settingInvoice */
        $settingInvoice = $subject->getEntity();
        ...
        if ($message !== '') {
            ...
            $message = 'System Parameters (' . $settingInvoice->AccountId . ') modified: ' . $message;
            $editorUserId = $this->getUserLoader()->loadSystemUserId();
            $this->getAuditTrailLogger()
                ->setAccountId($settingInvoice->AccountId)
                ->setEditorUserId($editorUserId)
                ->setEvent($message)
                ->setSection($section)
                ->setUserId($settingInvoice->ModifiedBy)
                ->log();
        }
    }
}
