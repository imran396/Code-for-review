...
namespace Sam\Observer\SettingBillingAuthorizeNet\Internal;
 ...
 * @package Sam\Observer\SettingBillingAuthorizeNet\Internal
 */
    ...
    implements EntityCreationObserverHandlerInterface, EntityUpdateObserverHandlerInterface
{
    ...
    {
        /** @var SettingBillingAuthorizeNet $settingBillingOpayo */
        $settingBillingOpayo = $subject->getEntity();
        ...
        if ($message !== '') {
            ...
            $message = 'System Parameters (' . $settingBillingOpayo->AccountId . ') modified: ' . $message;
            $editorUserId = $this->getUserLoader()->loadSystemUserId();
            $this->getAuditTrailLogger()
                ->setAccountId($settingBillingOpayo->AccountId)
                ->setEditorUserId($editorUserId)
                ->setEvent($message)
                ->setSection($section)
                ->setUserId($settingBillingOpayo->ModifiedBy)
                ->log();
        }
    }
}
