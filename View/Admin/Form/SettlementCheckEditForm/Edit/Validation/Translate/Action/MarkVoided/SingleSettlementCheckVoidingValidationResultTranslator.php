...
namespace Sam\View\Admin\Form\SettlementCheckEditForm\Edit\Validation\Translate\Action\MarkVoided;
...
/**
 * Class SingleSettlementCheckVoidingValidationResultTranslator
 * @package Sam\Settlement\Check\Action\MarkVoided\Single\Validate\Result
 */
class SingleSettlementCheckVoidingValidationResultTranslator extends CustomizableClass
{
    ...
    /** @var string[] */
    protected array $warningTranslations = [];
    ...
    public function construct(Result $validationResult, string $language): static
    {
        ...
        $this->buildWarningTranslationCollection();
        return $this;
    }
    ...
    public function warningMessage(?string $glue = null): string
    {
        $messages = array_map(
            function (ResultStatus $resultStatus) {
                return $this->transWarning($resultStatus->getCode());
            },
            $this->validationResult->getWarningStatuses()
        );
        $output = implode($glue, $messages);
        return $output;
    }
    ...
    protected function transWarning(int $code): string
    {
        if (!$this->hasWarningTranslation($code)) {
            return "Unable to find translation for warning code: [{$code}]";
        }
        return $this->warningTranslations[$code];
    }

    protected function hasWarningTranslation(int $code): bool
    {
        return isset($this->warningTranslations[$code]);
    }

    protected function buildErrorTranslationCollection(): void
    {
        $tr = $this->getAdminTranslator();
        $domain = 'admin_single_settlement_check_voiding_validator';
    ...
    protected function buildWarningTranslationCollection(): void
    {
        $tr = $this->getAdminTranslator();
        $domain = 'admin_single_settlement_check_voiding_validator';
        $this->warningTranslations = [
            Result::WARN_PAYMENT_APPLIED => $tr->trans('warning.payment_applied', [], $domain, $this->language)
        ];
    }
}
