...
use Sam\Details\Core\ConfigManager;
 ...
 * @package Sam\Details
 */
class CoreHintRenderer extends CustomizableClass
{
    ...
    public function construct(ConfigManager $configManager): CoreHintRenderer
    {
        $this->setConfigManager($configManager);
        return $this;
    }

    public function render(array $optionals = []): string
    {
        $this->beginEndKey = $optionals['beginEndKey'] ?? '';
        $this->compositeView = $optionals['compositeView'] ?? '';
        $this->inlineHelpSection = $optionals['inlineHelpSection'] ?? '';
        $this->isOptionSection = $optionals['isOptionSection'] ?? true;
        $this->isSinglePlaceholderPerLine = $optionals['isSinglePlaceholderPerLine'] ?? false;
...
HTML;
    ...
    protected function renderBeginEndSection(): string
    {
        ...
        $beginLogicalNot = $this->getConfigManager()->makeBeginKey($key, true);
        ...
        $endLogicalNot = $this->getConfigManager()->makeEndKey($key, true);
