...
use Sam\Invoice\Legacy\Charge\Admin\InvoiceEdit\Gate\AuthorizeNet\Internal\OtherCcCharge\AdminInvoiceEditAuthorizeNetOtherCcChargerResult as Result;
use Sam\Invoice\Legacy\Charge\Admin\InvoiceEdit\Gate\AuthorizeNet\Internal\OtherCcCharge\Internal\Load\DataProviderCreateTrait;
use Sam\Invoice\Legacy\Charge\Admin\InvoiceEdit\Gate\Common\AdminInvoiceEditChargingHelperCreateTrait;

class AdminInvoiceEditAuthorizeNetOtherCcCharger extends CustomizableClass
{
    ...
    public function charge(
        ...
        $ccExpDate = $this->createCcExpiryDateBuilder()->implode($expMonth, $expYear);
        ...
        if ($authNetManager->isTransactionInReview()) {
            $note = 'Payment with Trans.: ' . $authNetManager->getTransactionId() . ' was marked as "Held for Review"';
            return Result::new()->construct($note)->addSuccess(Result::OK_HELD_FOR_REVIEW);
        }

        $note = 'Trans.:' . $authNetManager->getTransactionId() . ' CC:' . substr($ccNumber, -4);
