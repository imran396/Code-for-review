...
namespace Sam\Invoice\StackedTax\Generate\Chunk\Internal\Report\Build;
...
use Sam\Invoice\StackedTax\Generate\Chunk\StackedTaxInvoiceChunkGenerationResult;
...
class ReportBuilder extends CustomizableClass
{
    ...
    public function completeReport(bool $isAccountFiltering, StackedTaxInvoiceChunkGenerationResult $result): string
    {
        $glue = ' <br />';
        $output = '';
        ...
        foreach ($result->generatedCounts as $accountId => $generatedCount) {
            $reportLines = [];
            $reportLines[] = $this->translate("invoice_generation.report.invoice_generated_in_total", ['invoiceCount' => $generatedCount]);
