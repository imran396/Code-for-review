...
use Sam\Invoice\StackedTax\Charge\Admin\PaymentEdit\Gate\AuthorizeNet\Internal\CimCharge\CimChargingResult as Result;
use Sam\Invoice\StackedTax\Charge\Admin\PaymentEdit\Gate\AuthorizeNet\Internal\CimCharge\Internal\Load\DataProviderCreateTrait;

class CimCharger extends CustomizableClass
{
    ...
    public function charge(
        ...
        if (!$authNetCimManager->isSuccessful()) {
            ...
            return Result::new()
                ->construct()
                ->addError(Result::ERR_CHARGE, $errorMessage);
        }

        if ($authNetCimManager->isTransactionInReview()) {
            $noteInfo = [
                'Trans.ID' => $authNetCimManager->getTransactionId(),
